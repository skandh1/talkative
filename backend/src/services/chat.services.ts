...
export class ChatService {
  ...
  static async getOrCreate1to1(userId: string, peerUserId: string): Promise<IConversation> {
    ...
    // Find a conversation that has BOTH user IDs in its participants array
    // AND has exactly two participants (to ensure it's a 1:1 chat).
    let conversation = await Conversation.findOne({
      participants: { 
        $all: [userObjectId, peerObjectId] 
      },
      'participants.2': { $exists: false } // Ensures exactly two participants
    });

    if (!conversation) {
      // If the conversation is new, the participants should still be sorted
      // to maintain a consistent structure, especially if you had a 
      // unique index on the sorted array (now removed).
      const participants = [userObjectId, peerObjectId].sort((a, b) => a.toString().localeCompare(b.toString()));
  ...
  static async sendMessage(
    ...
    // ✅ Return both message and conversation (so controller can broadcast to participants)
